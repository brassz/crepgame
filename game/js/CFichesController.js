function CFichesController(oContainer){
    ...
    this.setFicheOnButton = function(iIndexFicheSelected,szNameAttach,aMcFiches){
        // Função específica para colocar fichas no botão "APOSTE AQUI"
        this.addFicheOnButton(iIndexFicheSelected,szNameAttach,aMcFiches);
    };
    ...
    this.addFicheOnButton = function(iIndexFicheSelected,szNameAttach,aMcFiches){
        if(_aValueFichesInPos[szNameAttach] === undefined){
            _aValueFichesInPos[szNameAttach] = 0;
        }
        ...
        // Posição específica para o botão "APOSTE AQUI" (670, 480)
        var iXPos = 670; // Posição X do botão
        var iYPos = 480; // Posição Y do botão
        for(var k=0;k<aFiches.length;k++){
                aMcFiches.push(this._attachFichesPile(aFiches[k],szNameAttach,iXPos,iYPos));
                iYPos-=5;
        }
    ...
    this.removeBet = function(szName){
        // Remove as fichas visualmente da mesa
        if(_aMcFichesInPos[szName]){
            for(var i = 0; i < _aMcFichesInPos[szName].length; i++){
                var oFiche = _aMcFichesInPos[szName][i];
                if(oFiche && oFiche.getSprite() && _oContainer.contains(oFiche.getSprite())){
                    _oContainer.removeChild(oFiche.getSprite());
                }
            }
        }
        
        // Limpa os arrays internos
