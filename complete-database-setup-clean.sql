-- ==============================================
-- CLEAN SUPABASE DATABASE SETUP FOR CRAPS GAME
-- ==============================================
-- This version avoids function name conflicts
-- Execute this entire file in your Supabase SQL Editor

-- ==============================================
-- STEP 0: CLEANUP EXISTING FUNCTIONS (if any)
-- ==============================================
...
DROP FUNCTION IF EXISTS public.join_room_simple(TEXT);
DROP FUNCTION IF EXISTS public.join_room_turn_cycle(TEXT);
DROP FUNCTION IF EXISTS public.handle_dice_roll(TEXT, INTEGER, INTEGER, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.handle_dice_roll_simple(TEXT, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS public.complete_dice_animation(BIGINT);
...
-- Table for managing room sessions
...
-- ==============================================
-- STEP 3: CREATE FUNCTIONS (with unique names)
-- ==============================================

-- Function to join a room session (unique name to avoid conflicts)
CREATE OR REPLACE FUNCTION public.craps_join_room_session(p_room_id TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION public.craps_handle_dice_roll(
    p_room_id TEXT,
    p_dice_1 INTEGER,
    p_dice_2 INTEGER,
...
CREATE OR REPLACE FUNCTION public.craps_join_turn_cycle(p_room_id TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    PERFORM public.craps_join_room_session(p_room_id);
...
CREATE OR REPLACE FUNCTION public.craps_complete_animation(p_move_id BIGINT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION public.craps_leave_room(p_room_id TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION public.craps_join_room_session TO authenticated;
GRANT EXECUTE ON FUNCTION public.craps_handle_dice_roll TO authenticated;
GRANT EXECUTE ON FUNCTION public.craps_join_turn_cycle TO authenticated;
GRANT EXECUTE ON FUNCTION public.craps_complete_animation TO authenticated;
GRANT EXECUTE ON FUNCTION public.craps_leave_room TO authenticated;
...
-- Function names are prefixed with 'craps_' to avoid conflicts

-- ==============================================
-- FUNCTION REFERENCE
-- ==============================================
-- Use these function names in your application:
-- - public.craps_join_room_session(room_id)
-- - public.craps_handle_dice_roll(room_id, dice_1, dice_2, phase?, result?)
-- - public.craps_join_turn_cycle(room_id)
-- - public.craps_complete_animation(move_id)
-- - public.craps_leave_room(room_id)