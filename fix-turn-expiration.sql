-- Fix for "Turn has expired" error
-- This script modifies the turn management to be more lenient and user-friendly

-- 1. Extend turn duration from 25 to 45 seconds for better UX
-- 2. Add a function to refresh/extend turns when needed
-- 3. Modify handle_dice_roll_simple to auto-extend expired turns in some cases
...
-- Modified handle_dice_roll_simple with more lenient turn expiration
...
BEGIN
    ...
    -- Atualizar turno atual para o próximo jogador com 45 segundos (instead of 25)
    UPDATE public.current_turn 
    SET 
