...
// State (em produção, isso deveria usar Redis ou similar)
const TURN_MS = 25000;
const roomState = {
...
function emitTurnUpdate(room, io){
  const state = roomState[room];
  ...
  if (res.socket.server.io) {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('request_roll', () => {
        ...
        performRoll(currentRoom, io);
        startNextTurn(currentRoom, io);
      });
      ...
      socket.on('disconnect', () => {
        if (currentRoom) {
          ...
          if (state.order.length === 0){
            ...
            if (!cur) startNextTurn(currentRoom, io);
          }
        }
      });
    });
