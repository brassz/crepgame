function CInterface(){
    ...
    this._init = function(){
        ...
        // INFORMAÇÕES DA SALA - POSICIONADO PARA 1280x768
        var oRoomInfoBg = createBitmap(s_oSpriteLibrary.getSprite('display_bg'));
        oRoomInfoBg.x = 300;  // Posição central superior
        ...
        // Room selection button - posicionado para ficar visível em 1280x768
        // Posição calculada para garantir visibilidade na resolução padrão
        var iRoomButtonX = Math.min(1100, CANVAS_WIDTH - 140); // Garantir que não saia da tela
        var iRoomButtonY = 120;
        _pStartPosRoomSelection = {x: iRoomButtonX, y: iRoomButtonY};
        
        _oRoomSelectionBut = new CTextButton(_pStartPosRoomSelection.x, _pStartPosRoomSelection.y, s_oSpriteLibrary.getSprite('but_bg'), "SALAS", FONT1, "#fff", 16, "center", s_oStage);
        _oRoomSelectionBut.addEventListener(ON_MOUSE_UP, this._onRoomSelection, this);
        
        console.log("Botão SALAS criado na posição:", _pStartPosRoomSelection.x, _pStartPosRoomSelection.y);
    ...
    this.refreshButtonPos = function (iNewX, iNewY) {
        ...
        // Ajustar posição do botão de salas se necessário
        if(_oRoomSelectionBut){
            _oRoomSelectionBut.setPosition(_pStartPosRoomSelection.x - iNewX, _pStartPosRoomSelection.y + iNewY);
        }
    };
    ...
    this._onRoomSelection = function(){
        ...
        if(_oRoomSelectionPanel){
            ...
            console.error("_oRoomSelectionPanel não está definido!");
        }
    };
