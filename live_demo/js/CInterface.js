function CInterface(){
    ...
    this._init = function(){
        ...
        _oChangeSalaButton = new CTextButton(600, 120, s_oSpriteLibrary.getSprite('but_bg'), "TROCAR SALA", FONT1, "#fff", 14, "center", s_oStage);
        _oChangeSalaButton.addEventListener(ON_MOUSE_UP, this._onChangeSala, this);
    ...
    this.updateRoomInfo = function(sRoomType, iPlayers){
        if(_oRoomInfoText){
            ...
            sRoomInfo += "APOSTA MAX: R$" + oRoomConfig.max_bet;
            _oRoomInfoText.refreshText(sRoomInfo);
        }
        
        // Update bet limits display
        if(_oMsgTitle){
            var sBetInfo = TEXT_MIN_BET + ": R$" + MIN_BET + "\n" + TEXT_MAX_BET + ": R$" + MAX_BET;
            _oMsgTitle.refreshText(sBetInfo);
        }
    };
