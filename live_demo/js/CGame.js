function CGame(oData){
    ...
    var _oSalaSelector;
    var _sCurrentSalaId; // ID da sala atual
    var _sCurrentRoomType; // Tipo da sala atual
    ...
    this._init = function(){
        ...
        _oSalaSelector = new CSalaSelector();
    ...
    this._onSitDown = function(){
        // Mostrar seletor de salas em vez de entrar diretamente na mesa
        this._showSalaSelector();
    };
    
    this._showSalaSelector = function(){
        _oSalaSelector.show(this._onSalaSelected.bind(this), this._onSalaSelectorClosed.bind(this));
    };
    
    this._onSalaSelected = function(oSala){
        _sCurrentSalaId = oSala.id;
        _sCurrentRoomType = oSala.type;
        
        // Join the selected sala
        s_oRoomConfig.joinSala(_sCurrentSalaId);
        ...
        // Update interface with current sala info
        var oCurrentSala = s_oRoomConfig.getRoomConfig(_sCurrentSalaId);
        _oInterface.updateRoomInfo(_sCurrentSalaId, oCurrentSala.current_players);
        
        // Update global bet limits based on sala
        MIN_BET = oCurrentSala.min_bet;
        MAX_BET = oCurrentSala.max_bet;
        
        console.log("Entrou na sala:", oCurrentSala.name, "Limites:", MIN_BET, "-", MAX_BET);
    };
    
    this._onSalaSelectorClosed = function(){
        // If no sala was selected, exit to main menu
        this.onExit(true);
    };
    
    this.showSalaSelector = function(){
        // Clear current bet before showing selector
        if(_oMySeat.getCurBet() > 0){
            this.onClearAllBets();
        }
        ...
        // Leave current sala
        if(_sCurrentSalaId){
            s_oRoomConfig.leaveSala(_sCurrentSalaId);
        }
    ...
    this.onExit = function(bForceExit){
        // Leave current sala when exiting
        if(_sCurrentSalaId){
            s_oRoomConfig.leaveSala(_sCurrentSalaId);
        }
