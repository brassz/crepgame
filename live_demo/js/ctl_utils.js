...
function playSound(szSound,iVolume,bLoop){
    try {
        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            // Verificar se o som existe e está carregado
            if(s_aSounds && s_aSounds[szSound] && typeof s_aSounds[szSound].play === 'function'){
                // Verificar se o som está realmente carregado
                if(s_aSounds[szSound]._sounds && s_aSounds[szSound]._sounds.length > 0){
                    s_aSounds[szSound].play();
                    
                    // Verificar se volume é válido
                    if(iVolume !== undefined && typeof s_aSounds[szSound].volume === 'function'){
                        s_aSounds[szSound].volume(iVolume);
                    }
                    
                    // Verificar se loop é válido
                    if(bLoop !== undefined && typeof s_aSounds[szSound].loop === 'function'){
                        s_aSounds[szSound].loop(bLoop);
                    }
                    
                    return s_aSounds[szSound];
                } else {
                    console.warn("Som não está carregado ainda:", szSound);
                    return null;
                }
            } else {
                console.warn("Som não encontrado:", szSound);
                return null;
            }
        }
        return null;
    } catch(error) {
        console.error("Erro ao reproduzir som:", szSound, error);
        return null;
    }
}
...
function stopSound(szSound){
    try {
        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            if(s_aSounds && s_aSounds[szSound] && typeof s_aSounds[szSound].stop === 'function'){
                s_aSounds[szSound].stop();
            }
        }
    } catch(error) {
        console.error("Erro ao parar som:", szSound, error);
    }
}   
...
function setVolume(szSound, iVolume){
    try {
        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            if(s_aSounds && s_aSounds[szSound] && typeof s_aSounds[szSound].volume === 'function'){
                s_aSounds[szSound].volume(iVolume);
            }
        }
    } catch(error) {
        console.error("Erro ao definir volume do som:", szSound, error);
    }
}  
...
function setMute(szSound, bMute){
    try {
        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            if(s_aSounds && s_aSounds[szSound] && typeof s_aSounds[szSound].mute === 'function'){
                s_aSounds[szSound].mute(bMute);
            }
        }
    } catch(error) {
        console.error("Erro ao definir mute do som:", szSound, error);
    }
}
